...
!include 'mpif.h'
  ...
  double precision,dimension(:),allocatable :: sa,b,f,r,ref_x,x,x_temp,full_x,G,Gj, g_cap,x_cap, x_top, iseed
  integer,dimension(:),allocatable :: isa,jsa,ipiv
  character(len=100) :: name
  character(len=2) :: uplo,proc
  character(len=10000000) :: order, band_width
  integer :: P, bw, nb_process
  !! for banded solver
  double precision,dimension(:,:),allocatable :: ba, ba_temp, ba_lu, Aj,spk_cap_mat, spk_temp,Bj, Cj,Bjtemp,Cjtemp, Vj_mat, Wj_mat, Vj, Wj, Vj_Wj_Gj
...
call MPI_INIT(ierr)
call MPI_COMM_SIZE(MPI_COMM_WORLD,nb_process,ierr)
...
if(rank==0) then
  ...
  read(band_width, '(i)') bw
  print *, "Bandwidth: ", bw
  kl = (bw-1)/2
  ku = (bw-1)/2
  print *, "Kl and Ku: ", (bw-1)/2
...
!!!!!!!!!!!!!!
  ...
  allocate(r(1:N)) !! residual array
     ...
     allocate(ba_temp(1:kl+ku+1,1:N))
     allocate(iseed(4))
     iseed = [100,101,102,103]
     ba=0.0d0 ! all elements at zero
     do i=1,bw
       call DLARNV(2, iseed, N, ba(i,:))
     enddo
     ba_temp = ba
