...
include 'mpif.h'
  ...
  integer :: t1,t2,tim,proc,rank,code
  ...
  integer :: P
  !! for banded solver
  double precision,dimension(:,:),allocatable :: ba, ba_lu, Aj, spk_cap_mat, Bj, Cj, Vj_mat, Vj_mat1,Wj_mat, Vj, Wj
...
call MPI_INIT(code)
call MPI_COMM_SIZE(MPI_COMM_WORLD,proc,code)
...
if(rank==0)then
!print *,"In ",rank," Aj is", Aj
do k=2,P
   Aj = ba(1:kl+ku+1,(1+(k-1)*Nj):(k*Nj))
   call MPI_SEND(Aj,Nj*(kl+ku+1),MPI_DOUBLE_PRECISION,k-1,tag,MPI_COMM_WORLD,code)
end do
k=1
Aj = ba(1:kl+ku+1,(1+(k-1)*Nj):(k*Nj))
endif
...
if(rank.ne.0) then
call MPI_RECV(Aj,Nj*(kl+ku+1),MPI_DOUBLE_PRECISION,0,tag,MPI_COMM_WORLD,status,code)
!print *,"In ",rank," Aj is", Aj
end if
...
if(rank==0) then
k=1
Vj_mat1(Nj-M+1:Nj,1+((k-1)*M):k*M) = Bj
!print *,"for ",rank," initial Vj is ",Vj_mat
!LU SOlve
call DTBSM('L','N','U',Nj,M,kl,Aj(ku+1,1),kl+ku+1,Vj_mat1(:,1+((k-1)*M):k*M),Nj)
call DTBSM('U','N','N',Nj,M,ku,Aj,kl+ku+1,Vj_mat1(:,1+((k-1)*M):k*M),Nj)
!print *,"for ",rank," Vj_mat is ",Vj_mat
!Storing Vj bottom in reduced spike matrix
spk_cap_mat(1+(k-1)*2*M : 1+(k-1)*2*M+(M-1), (2*k*M)-(M-1) : 2*k*M) = Vj_mat1(Nj-M+1:Nj, 1+((k-1)*M):k*M)
!Storing Vj top in reduced spike matrix
if(k>1) then
spk_cap_mat((2*k-3)*M+1 : (2*k-2)*M, (2*k*M)-(M-1) : 2*k*M) = Vj_mat1(1:M, 1+((k-1)*M):k*M)
end if
endif
...
!print *,"for k=",k," Wj_mat is",Wj_mat
end if

!if not main processor, pass respective, vj and wj to spk_cap_mat
if(rank.ne.0) then
k=rank+1
!print *,"SENDING values for k =",k
call MPI_SEND(k,1,MPI_INTEGER,0,tag,MPI_COMM_WORLD,code)
print *,"Sent k"
!print *,"here"
call MPI_SEND(Vj_mat,Nj*P*M,MPI_DOUBLE_PRECISION,0,tag,MPI_COMM_WORLD,code)
print *,"Sent Vj"
call MPI_SEND(Wj_mat,Nj*P*M,MPI_DOUBLE_PRECISION,0,tag,MPI_COMM_WORLD,code)
print *,"Sent Wj"
end if
...
call MPI_RECV(k,1,MPI_INTEGER,1,tag,MPI_COMM_WORLD,status,code)
print *,"Received k is ",k
call MPI_RECV(Vj_mat,Nj*P*M,MPI_DOUBLE_PRECISION,k-1,tag,MPI_COMM_WORLD,status,code)
print *,"Received Vj"
call MPI_RECV(Wj_mat,Nj*P*M,MPI_DOUBLE_PRECISION,k-1,tag,MPI_COMM_WORLD,status,code)
print *,"Received Wj"
!Storing Vj bottom in reduced spike matrix
...
enddo
!print *,"spk_cap_mat is ",spk_cap_mat
end if
...
!print *, "Gj is", Gj
call MPI_SEND(k,1,MPI_INTEGER,0,tag,MPI_COMM_WORLD,code)
call MPI_SEND(Gj,Nj,MPI_DOUBLE_PRECISION,0,tag,MPI_COMM_WORLD,code)
!print *,"Sent Gj for k=",k
endif
...
call DTBSM('U','N','N',Nj,1,ku,Aj,kl+ku+1,Gj,Nj)
G(1+(k-1)*Nj:k*Nj) = Gj
!print *,"Gj of 1 is done"
call MPI_RECV(k,1,MPI_INTEGER,1,tag,MPI_COMM_WORLD,status,code)
!print *,"Receiving Gj for k= ",k
Gj=0.0d0
!print *,"Nj is",Nj
call MPI_RECV(Gj,Nj,MPI_DOUBLE_PRECISION,1,tag,MPI_COMM_WORLD,status,code)
!print *,"Received Gj is ",Gj
G(1+(k-1)*Nj:k*Nj) = Gj
!print *,"Written into G"
!print *,"G is ",G
...
!print *,'info',info
!print *,"x_cap is ",x_cap
!send x_cap to others
do i=2,P
call MPI_SEND(x_cap,s_cap_size,MPI_DOUBLE_PRECISION,i-1,tag,MPI_COMM_WORLD,code)
end do
call MPI_SEND(G,N,MPI_DOUBLE_PRECISION,1,tag,MPI_COMM_WORLD,code)
!print *,"Printing G =",G
!calculate one part in main
k=1
allocate(Vj(Nj,M))
allocate(Wj(Nj,M))
allocate(x_top(M))
...
!1, M, -1.0d0, Wj, Nj, x_cap(1+2*(k-2)*M:((2*K)-3)*M), M, 1.0d0, x(1+(k-1)*Nj:k*Nj), Nj) !TODO: Check if LDA, LDB and LDC for parameters are correct
print *,"x that was calculated in main is ",x(1+(k-1)*Nj:k*Nj)
endif

!accept x_cap and do calculations to retrieve x
call MPI_BARRIER(MPI_COMM_WORLD,code)
if(rank.ne.0) then
k=rank+1
allocate(x(1:N))
...
x_top=0.0d0
s_cap_size = 2*(P-1)*M
allocate(x_cap(1:s_cap_size))
!print *,"right before receive"
call MPI_RECV(x_cap,s_cap_size,MPI_DOUBLE_PRECISION,0,tag,MPI_COMM_WORLD,status,code)
allocate(G(N))
call MPI_RECV(G,N,MPI_DOUBLE_PRECISION,0,tag,MPI_COMM_WORLD,status,code)
x(1+(k-1)*Nj:k*Nj) = g(1+((k-1)*Nj):k*Nj)
...
print *,"X that was calculated in processor ",rank+1," is ",x(1+(k-1)*Nj:k*Nj)
endif
...
!! Timer ends
 ! call system_clock(t2,tim) ! final time
!if(rank==0) then
!  print *,'Total time for our solution is ',(t2-t1)*1.0d0/tim
!endif
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
