...
!include 'mpif.h'
  ...
  integer :: i,j,ierr,N,k,Nx,Ny,e,nnz,rS,cS, Nj,M, s_cap_size
  integer :: t1,t2,tim,rank,code,flag
  double precision :: L,nres,err, error
  double precision,dimension(:),allocatable :: sa,b,f,r,ref_x,x,x_temp,full_x,G,Gj, g_cap,x_cap, x_top
  integer,dimension(:),allocatable :: isa,jsa,ipiv
  character(len=100) :: name
  character(len=1) :: uplo,proc
  integer :: P
  !! for banded solver
  double precision,dimension(:,:),allocatable :: ba, ba_lu, Aj,spk_cap_mat, spk_temp,Bj, Cj,Bjtemp,Cjtemp, Vj_mat, Wj_mat, Vj, Wj, Vj_Wj_Gj
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!IMPORTANT COMMENT : WHATEVER IS NOT IN AN IF-THEN LOOP, IT IS COMMON TO ALL PROCESSORS

!Note1.Common calculation
  call getarg(1,name)
  call getarg(2,proc)
  read(proc,'(i)')p
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!     print *, "Reference Solution is:", ref_x
  ...
  call system_clock(t1,tim) ! final time
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

!  print *,'Total time for reference solution',(t2-t1)*1.0d0/tim
...
!Note2. Till now, basic calculations were done by 1st processors. kl and ku needed by everyone, so broadcast
!call MPI_BCAST(kl,1,MPI_INTEGER,0,MPI_COMM_WORLD,code)
!call MPI_BCAST(ku,1,MPI_INTEGER,0,MPI_COMM_WORLD,code)

do i=1,P-1
 if(rank==0) then
  call MPI_SEND(kl,1,MPI_INTEGER,i,tag,MPI_COMM_WORLD,ierr)
 endif
 if(rank==i) then
  call MPI_RECV(kl,1,MPI_INTEGER,0,tag,MPI_COMM_WORLD,status,ierr)
 endif
enddo

do i=1,P-1
 if(rank==0) then
  call MPI_SEND(ku,1,MPI_INTEGER,i,tag,MPI_COMM_WORLD,ierr)
 endif
 if(rank==i)then
  call MPI_RECV(ku,1,MPI_INTEGER,0,tag,MPI_COMM_WORLD,status,ierr)
 endif
enddo

print *,"Rank = ",rank,"kl =",kl,"ku =",ku
!! Timer Starts
  call system_clock(t1,tim) ! initialize time
...
nzero=0.0d0
 ...
 allocate(Vj_mat(Nj,M))
 allocate(Wj_mat(Nj, M))
 allocate(Bjtemp(M,M))
 allocate(Cjtemp(M,M)) 
 !ba_lu = 0.0d0
 Bj = 0.0d0
...
do k=2,P
   ...
   call MPI_SEND(Aj,Nj*(kl+ku+1),MPI_DOUBLE_PRECISION,k-1,tag,MPI_COMM_WORLD,ierr)
end do
k=1
Aj = ba(1:kl+ku+1,(1+(k-1)*Nj):(k*Nj))
endif
...
if(rank.ne.0) then
  call MPI_RECV(Aj,Nj*(kl+ku+1),MPI_DOUBLE_PRECISION,0,tag,MPI_COMM_WORLD,status,ierr)
end if
...
Bjtemp = 0.0d0
call DLACPY('L', ku, ku, Aj(1,1), kl+ku, Bjtemp, ku) !! When in doubt, check this :p
!print *, "Bj of ",k," is", Bj

!TODO: check if logic works
do i=1,P-1
  if(rank==i) then
    call MPI_SEND(Bjtemp,ku*ku,MPI_DOUBLE_PRECISION,i-1,tag,MPI_COMM_WORLD,ierr)
  endif
  if(rank==i-1) then
    call MPI_RECV(Bj,ku*ku,MPI_DOUBLE_PRECISION,i,tag,MPI_COMM_WORLD,status,ierr)
  endif
end do

!print *,"Everyone has correct Bj" 


call DLACPY('U', kl, kl, Aj(kl+ku+1,(1+Nj-kl)), kl+ku, Cjtemp, ku) !! When in doubt, check this :p

!TODO : check logic 
do i=0,P-2
  if(rank==i) then
    call MPI_SEND(Cjtemp,ku*ku,MPI_DOUBLE_PRECISION,i+1,tag,MPI_COMM_WORLD,ierr)
  endif
  if(rank==i+1) then
    call MPI_RECV(Cj,ku*ku,MPI_DOUBLE_PRECISION,i,tag,MPI_COMM_WORLD,status,ierr)
  endif
end do

!print *,"everyone has their own Cj , i.e., correct cj"
allocate(Gj(Nj))
Gj = 0.0d0
allocate(Vj_Wj_Gj(Nj, (2*M)+1))
!Vj_Wj_Gj = 0.0d0
!Vj_Wj_Gj(Nj-M+1:Nj, 1:M) = Bj
!Vj_Wj_Gj(1:M, (M+1):2*M) = Cj
!print *, "rank", rank, "Bj is", Bj
!print *, "rank", rank, "Cj is", Cj
!Vj_Wj_Gj(:,(2*M)+1) = 1.0d0 !Assuming RHS/F/B will be 1.0d0
Vj_mat(Nj-M+1:Nj,1:M) = Bj
Wj_mat(1:M, 1:M) = Cj
Gj = 1.0d0
!LU SOlve
call DTBSM('L','N','U',Nj,M,kl,Aj(ku+1,1),kl+ku+1,Vj_mat,Nj)
call DTBSM('U','N','N',Nj,M,ku,Aj,kl+ku+1,Vj_mat,Nj)

call DTBSM('L','N','U',Nj,M,kl,Aj(ku+1,1),kl+ku+1,Wj_mat,Nj)
call DTBSM('U','N','N',Nj,M,ku,Aj,kl+ku+1,Wj_mat,Nj)

call DTBSM('L','N','U',Nj,1,kl,Aj(ku+1,1),kl+ku+1,Gj,Nj)
call DTBSM('U','N','N',Nj,1,ku,Aj,kl+ku+1,Gj,Nj)

!Vj_mat = Vj_Wj_Gj(:, 1:M)
!Wj_mat = Vj_Wj_Gj(:, (M+1):2*M)
!Gj = Vj_Wj_Gj(:,(2*M)+1)
!print *, "rank", rank, "Vj is", Vj_mat
!print *, "rank", rank, "Wj is", Wj_mat
!print *, "rank", rank, "Gj is", Gj
!print *, "Gj is", Gj
!they all have their own Aj,Bj,Vj,Wj
!everyone should store in their own spk_cap_mat

!Storing Vj bottom in reduced spike matrix
if (k <P) then
  print *,"1 for rank ",rank," is ",1+(k-1)*2*M
  print *,"1 for rank ",rank," is ",1+(k-1)*2*M+(M-1)
  print *,"1 for rank ",rank," is ",(2*k*M)-(M-1)
  print *,"1 for rank ",rank," is ",2*k*M
 !print *,"this was done"
spk_cap_mat(1+(k-1)*2*M : 1+(k-1)*2*M+(M-1), (2*k*M)-(M-1) : 2*k*M) = Vj_mat(Nj-M+1:Nj, 1:M)
!print *,"1 for rank ",rank," is ",1+(k-1)*2*M
!print *,"1 for rank ",rank," is ",1+(k-1)*2*M+(M-1)
!print *,"1 for rank ",rank," is ",(2*k*M)-(M-1)
!print *,"1 for rank ",rank," is ",2*k*M
...
!Storing Vj top in reduced spike matrix
  if(k>1) then
print *,"2 for rank ",rank," is ",(2*k-3)*M+1
 print *,"2 for rank ",rank," is ",(2*k-2)*M
 print *,"2 for rank ",rank," is ",(2*k*M)-(M-1)
 print *,"2 for rank ",rank,"is ",2*k*M
! print *,"2 was done"
    spk_cap_mat((2*k-3)*M+1 : (2*k-2)*M, (2*k*M)-(M-1) : 2*k*M) = Vj_mat(1:M, 1:M)
!print *,"2 for rank ",rank," is ",(2*k-3)*M+1
!print *,"2 for rank ",rank," is ",(2*k-2)*M
!print *,"2 for rank ",rank," is ",(2*k*M)-(M-1)
!print *,"2 for rank ",rank,"is ",2*k*M
...
!Storing Wj top in reduced spike matrix
 print *,"3 for rank ",rank," is ",(2*(k-1)*m)-(M-1)
  print *,"3 for rank ",rank," is ",2*(k-1)*M
  print *,"3 for rank ",rank," is ",1+(k-2)*2*M
  print *,"3 for rank ",rank,"is ",1+(k-2)*2*M+(M-1)
  ! print *,"2 was done"

  spk_cap_mat((2*(k-1)*m)-(M-1) : 2*(k-1)*M, 1+(k-2)*2*M : 1+(k-2)*2*M+(M-1)) = Wj_mat(1:M, 1:M)
!Storing Wj bottom in reduced spike matrix
  if(k<P) then
  print *,"4 for rank ",rank," is ",(2*(k-1)*M)+1
 print *,"4 for rank ",rank," is ",(2*k-1)*M
 print *,"4 for rank ",rank," is ",1+(k-2)*2*M
 print *,"4 for rank ",rank,"is ",1+(k-2)*2*M+(M-1)
    spk_cap_mat((2*(k-1)*M)+1 : (2*k-1)*M, 1+(k-2)*2*M : 1+(k-2)*2*M+(M-1)) = Wj_mat(Nj-M+1:Nj, 1:M)
  end if
end if
...
!if(rank==0) then
!print *,"rank =",rank," spk_cap_mat =",spk_cap_mat
!endif

! call MPI_BCAST(spk_cap_mat,s_cap_size*s_cap_size,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,code)

do i=1,P-1
 if(rank==0) then
  call MPI_SEND(spk_cap_mat,s_cap_size*s_cap_size,MPI_DOUBLE_PRECISION,i,tag,MPI_COMM_WORLD,ierr)
 endif
 if(rank==i)then
  call MPI_RECV(spk_cap_mat,s_cap_size*s_cap_size,MPI_DOUBLE_PRECISION,0,tag,MPI_COMM_WORLD,status,ierr)
 endif
enddo

!note. everyone calculates their Gj
allocate(G(N))
G=0.0d0
...
!note. Everyone except 0, will send to 0
do i=1,P-1
  if(rank==i) then
    call MPI_SEND(Gj,Nj,MPI_DOUBLE_PRECISION,0,tag,MPI_COMM_WORLD,ierr)
    ! print *,"Sending from r=",rank,"Gj = ",Gj(Nj-m+1:Nj)
  endif
  if(rank==0) then
    call MPI_RECV(Gj,Nj,MPI_DOUBLE_PRECISION,i,tag,MPI_COMM_WORLD,status,ierr)
    G(1+((i+1)-1)*Nj:(i+1)*Nj) = Gj
  endif
enddo


!note. all Gj's were received, and G was constructed

!call MPI_BCAST(G,N,MPI_DOUBLE_PRECISION,0,MPI_COMM_WORLD,code)

do i=1,P-1
 if(rank==0) then
  call MPI_SEND(G,N,MPI_DOUBLE_PRECISION,i,tag,MPI_COMM_WORLD,ierr)
 endif
 if(rank==i)then
  call MPI_RECV(G,N,MPI_DOUBLE_PRECISION,0,tag,MPI_COMM_WORLD,status,ierr)
 endif
enddo

!print *,"rank is ",rank,"G is ",G(Nj-m+1:Nj)
...
do i=1,P
  ...
  if(i<P) then
    ...
    !print *,"rhs is",G((i*Nj)-M+1 : (i*Nj))
  end if
  !To store Gt
  if(i>1) then
    g_cap(m*((2*i)-3)+1 : 2*M*(i-1)) = G(((i-1)*Nj)+1 : ((i-1)*Nj+M))
    !print *,"rhs2 inside",G(((i-1)*Nj)+1 : ((i-1)*Nj+M))
  end if
  !print *,"g_cap is done"
end do

!print *,"for rank=",rank," g_cap is ",g_cap
!print *,"rhs is ",G((1*Nj)-M+1 : (1*Nj))
!print *,"rhs 2 is",G(((2-1)*Nj)+1 : ((2-1)*Nj+M))

allocate(x(1:Nj)) !! solution
...
!print *,"rank =",rank,"x_cap is ",x_cap
...
if (rank /= (P-1)) then
  ...
  call DGEMM("N", "N", Nj, 1, M, -1.0d0, Vj_mat, Nj, x_top, M, 1.0d0, x, Nj) !TODO: Check if LDA, LDB and LDC for parameters are correct
end if
if (rank /= 0) then
  call DGEMM("N", "N", Nj, 1, M, -1.0d0, Wj_mat, Nj, x_cap(1+2*(k-2)*M:((2*K)-3)*M), M,1.0d0,x, Nj) !TODO: Check if LDA, LDB and LDC for parameters are correct
end if

!if(rank==1) then
!do i=1,Nj
!print *,"rank= ",rank,"i= ",i+216000,"x=",x(i)
!enddo
!end if
if(rank==0) then
allocate(x_temp(1:Nj))
allocate(full_x(1:N))
full_x=0.0d0
full_x(1:Nj)=x
endif
...
do i=1,P-1
if(rank==i) then
 call MPI_SEND(x,Nj,MPI_DOUBLE_PRECISION,0,tag,MPI_COMM_WORLD,ierr)
endif
if(rank==0) then
 call MPI_RECV(x_temp,Nj,MPI_DOUBLE_PRECISION,i,tag,MPI_COMM_WORLD,status,ierr)
 full_x(1+((i+1)-1)*Nj:(i+1)*Nj)=x_temp
endif
end do
...
if(rank==0) then
  r=b
  call dcsrmm(uplo,N,N,1,-1.0d0,sa,isa,jsa,full_x,1.0d0,r)
  nres=sum(abs(r))/sum(abs(b)) ! norm relative residual 
  print *, "Residual from nres is", nres
...
do i=0,p-1
if(rank==i) then
print *,"rank=",rank,"is going to call finalize"
call MPI_FINALIZE(ierr)
print *,"finalized for rank =",rank
endif
enddo
 
!call MPI_BARRIER(MPI_COMM_WORLD,ierr)

print *,"program is not parallel anymore"

end program spike_lu

!Zn reduced spike matrix
